...
if platform.system() == "Darwin":
   user_documents_path = str(Path.home() / "Documents")
   FILE_NAME = os.path.join(user_documents_path, "scraped_prices.xlsx")
else: 
...
# delete selected row from table and excel file
def delete_selected_row():
    """Deletes the selected row from the table and the Excel file."""
    selected_item = table.selection()  # Get the selected row in the table
    if selected_item:
        # Get the values of the selected row
        row_values = table.item(selected_item, "values")
        item_name = row_values[7]  # Assuming the "item" column is the seventh column

        # Delete from Excel
        if delete_row_from_excel(item_name):
            # Remove from Treeview
            table.delete(selected_item)
            display_message("Row deleted successfully!", "success")
        #else:
            #display_message("Failed to delete the row from Excel.", "error")
    else:
        display_message("No Selection: Please select a row to delete.", "error")

def delete_row_from_excel(item_name):
    """
    Deletes a row from the Excel sheet based on the item name.
    ...
    # Iterate through the rows to find the matching item name
    for row_idx in range(2, sheet.max_row + 1):  # Skip header row
        cell_value = sheet.cell(row=row_idx, column=8).value  # Column 7 holds the "url" field
        display_message(item_name, "success")
        display_message(cell_value, "success")
        if cell_value == item_name:
            sheet.delete_rows(row_idx)  # Delete the matching row
            row_deleted = True
            break

    if row_deleted:
        return save_excel(workbook)
    else:
        return False
...
input_frame.pack_propagate(False)  # Prevent the frame from resizing to fit its content

tk.Label(input_frame, text="Enter URL:", bg=POKEDEX_BLACK, fg=POKEDEX_WHITE, font=("Courier New", 10)).grid(row=0, column=0, padx=10)
url_entry = tk.Entry(input_frame, width=30, font=("Courier New", 10), bg=POKEDEX_GRAY, fg=POKEDEX_WHITE)
...
# Add Delete Button to the input_frame
delete_button = ttk.Button(input_frame, text="Delete Selected", command=delete_selected_row)
